...
def cosine_distance(input1, input2):
    ...
    """
    m, n = input1.size(0), input2.size(0)
    input1_sum = torch.pow(input1, 2).sum(dim=1, keepdim=True).expand(m, n)
    input2_sum = torch.pow(input2, 2).sum(dim=1, keepdim=True).expand(n, m).t()
    eps = 1e-12
    distmat = torch.mm(input1, input2.t()) / (torch.pow(input1_sum, .5) * torch.pow(input2_sum, .5) + eps)
    return distmat