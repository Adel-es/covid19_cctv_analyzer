...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        # print("in set index : ", index)
        img_path, pid, camid = self.data[index]
        img = read_image(img_path)
        # print("in set: ",img_path)
        # print("in set: ",img.size)
...
class QueryImageDatasetForConcat(Dataset):
    ...
    def __init__(self, img_file, root_dir, **kwargs):
        ...
        super(QueryImageDatasetForConcat, self).__init__(train, query, gallery, **kwargs)
    ...
    def process_dir(self, dir_path):
        ...
        img_paths = [dir_path +"/01.PNG", dir_path + "/002.PNG"]
...
class GalleryImageDatasetForConcat(Dataset):
    
    def __init__(self, gallery_imgs, root_dir, **kwargs):
        """
        Args:
            img_file (string): img 파일 경로
            root_dir (string): 모든 이미지가 존재하는 디렉토리 경로
            transform (callable, optional): 샘플에 적용될 Optional transform
        """
        self.gallery_imgs = gallary_imgs #read_image(img_file)
        self.root_dir = root_dir
